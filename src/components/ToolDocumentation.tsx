...
const toolDocumentation: ToolDoc[] = [
  ...
  {
    name: 'translate',
    description: 'Translate text between different languages with automatic language detection',
    icon: LanguageIcon,
    category: 'Utility',
    parameters: [
      {
  ...
  {
    name: 'get_time',
    description: 'Get current time for any timezone with flexible formatting options',
    icon: ClockIcon,
    category: 'Utility',
    parameters: [
      {
...
const ToolDocumentation: React.FC<ToolDocumentationProps> = ({ onClose }) => {
  const toolsByCategory = toolDocumentation.reduce((acc, tool) => {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm" onClick={onClose}>
      <div className="relative bg-white/10 backdrop-blur-xl border border-white/20 rounded-2xl shadow-2xl w-full max-w-4xl mx-auto p-8 overflow-y-auto max-h-[90vh]" onClick={e => e.stopPropagation()}>
        ...
        <p className="text-gray-300 mb-8">
          Learn about each available tool, their parameters, and how to use them effectively in your conversations.
        </p>
            ...
            <div key={category}>
              ...
              <div className="space-y-6">
                {tools.map((tool) => {
                  ...
                  return (
                    <div key={tool.name} className="bg-white/5 rounded-xl p-6 border border-white/10">
                      ...
                      {/* Parameters */}
                      <div className="mb-6">
                        <h5 className="text-sm font-semibold text-white mb-3">Parameters</h5>
                          ...
                          {tool.parameters.map((param) => (
                            <div key={param.name} className="flex items-start gap-3 text-sm">
                              <div className="flex items-center gap-2 min-w-0 flex-1">
                                ...
                                {param.required && (
                                  <span className="text-red-400 text-xs bg-red-400/10 px-2 py-0.5 rounded">Required</span>
                                )}
                              </div>
                              <div className="flex-1 text-gray-300">
                                <p>{param.description}</p>
                                {param.example && (
                                  <p className="text-xs text-gray-400 mt-1">Example: {param.example}</p>
                                )}
                              </div>
                            </div>
                          ))}
                      ...
                      {/* Examples */}
                      <div className="mb-6">
                        <h5 className="text-sm font-semibold text-white mb-3">Examples</h5>
                        <div className="space-y-3">
                          {tool.examples.map((example, index) => (
                            <div key={index} className="bg-black/20 rounded-lg p-3">
                              <p className="text-sm text-gray-300 mb-2">
                                <span className="text-blue-400 font-medium">User:</span> {example.prompt}
                              </p>
                              <p className="text-sm text-gray-300">
                                <span className="text-green-400 font-medium">Response:</span> {example.response}
                              </p>
                            </div>
                          ))}
                        </div>
                      ...
                      {/* Tips */}
                      <div>
                        <h5 className="text-sm font-semibold text-white mb-3">Tips</h5>
        ...
        <div className="mt-8 pt-6 border-t border-white/20">
          <div className="bg-blue-600/10 border border-blue-500/20 rounded-lg p-4">
            <h4 className="text-sm font-semibold text-blue-300 mb-2">How to Use Tools</h4>
            <ul className="text-sm text-gray-300 space-y-1">
              <li>• Enable tool calling using the toggle in the chat interface</li>
              <li>• Select which tools you want to use via the Tools button</li>
              <li>• Ask questions that would benefit from real-time data or calculations</li>
              <li>• The AI will automatically choose and use the appropriate tools</li>
              <li>• Tool results will appear in the conversation as tool messages</li>
            </ul>
          </div>
        </div>
      </div>
