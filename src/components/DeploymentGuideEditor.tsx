...
import {
  deploymentGuideService,
  DeploymentGuideService,
} from '../services/deploymentGuideService';
import type {
  DeploymentSection,
  DeploymentTab,
...
const DeploymentGuideEditor: React.FC<DeploymentGuideEditorProps> = ({
  ...
  const loadContent = () => {
    ...
    try {
      ...
      console.log(
        'First tab sections:',
        fullContent.categories[0]?.tabs[0]?.sections
      );
        ...
        category.tabs.forEach((tab, tabIndex) => {
          tab.sections.forEach((section, sectionIndex) => {
            if (section.type === 'code-block') {
              console.log(
                `Code block at category ${catIndex}, tab ${tabIndex}, section ${sectionIndex}:`,
                section
              );
  ...
  const getCurrentTab = (): DeploymentTab | undefined => {
    ...
    const category = content.categories.find(
      (cat: DeploymentCategory) => cat.id === selectedCategory
    );
    const tab = category?.tabs.find(
      (tab: DeploymentTab) => tab.id === selectedTab
    );
  ...
  const updateSection = (index: number, updatedSection: DeploymentSection) => {
    ...
    const categoryIndex = newContent.categories.findIndex(
      (cat: DeploymentCategory) => cat.id === selectedCategory
    );
    const tabIndex = newContent.categories[categoryIndex].tabs.findIndex(
      (tab: DeploymentTab) => tab.id === selectedTab
    );

    newContent.categories[categoryIndex].tabs[tabIndex].sections[index] =
      updatedSection;
  ...
  const addSection = () => {
    ...
    newContent.categories[categoryIndex].tabs[tabIndex].sections.push(
      newSection
    );
    setContent(newContent);
    setShowAddSection(false);
    setSelectedSection(
      newContent.categories[categoryIndex].tabs[tabIndex].sections.length - 1
    );
  };
  ...
  const deleteSection = (index: number) => {
    ...
    newContent.categories[categoryIndex].tabs[tabIndex].sections.splice(
      index,
      1
    );
    setContent(newContent);
        ...
        {isExpanded && (
          <div className='mt-2 p-4 bg-gray-800/30 border border-gray-600/30 rounded-lg'>
            ...
            <div className='mb-4'>
              ...
              >
                <option value='overview' className='bg-gray-800 text-white'>
                  Overview
                </option>
                <option value='requirements' className='bg-gray-800 text-white'>
                  Requirements
                </option>
                <option value='code-block' className='bg-gray-800 text-white'>
                  Code Block
                </option>
                <option value='providers' className='bg-gray-800 text-white'>
                  Providers
                </option>
                <option value='links' className='bg-gray-800 text-white'>
                  Links
                </option>
              </select>
            </div>
              ...
              <div className='space-y-4'>
                ...
                <div>
                  ...
                  <textarea
                    value={section.code || ''}
                    onChange={e =>
                      updateSection(index, { ...section, code: e.target.value })
                    }
                    rows={8}
                ...
                {section.subsections && section.subsections.length > 0 && (
                  <div>
                    ...
                    {section.subsections.map((subsection, subIndex: number) => (
                      <div
                        key={subIndex}
                        className='mb-3 p-3 bg-gray-700/30 border border-gray-600/30 rounded'
                      >
                ...
                <div>
                  ...
                  <textarea
                    value={
                      section.major
                        ?.map(
                          provider => `${provider.name} - ${provider.models}`
                        )
                        .join('\n') || ''
                    }
                    onChange={e => {
            ...
            {section.type === 'links' && (
              <div className='mb-4'>
                ...
                <textarea
                  ...
                  onChange={e => {
                    const newResources = e.target.value
                      .split('\n')
                      .map(line => {
  ...
  return (
    ...
    >
      <div
        className='bg-white/10 backdrop-blur-xl border border-white/20 rounded-2xl shadow-2xl w-full max-w-6xl h-[90vh] flex flex-col'
              ...
              <div>
                ...
                <select
                  ...
                  onChange={e => {
                    ...
                    if (!content) return;
                    const category = content.categories.find(
                      (cat: DeploymentCategory) => cat.id === e.target.value
                    );
