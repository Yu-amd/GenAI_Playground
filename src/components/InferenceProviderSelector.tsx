...
export const InferenceProviderSelector: React.FC<InferenceProviderSelectorProps> = ({
  ...
  const [currentProvider, setCurrentProvider] = useState<InferenceProvider>(getCurrentProvider());
  const [healthStatus, setHealthStatus] = useState<Record<InferenceProvider, {
    isHealthy: boolean;
    responseTime?: number;
    error?: string;
  }>>({
  ...
  const getHealthIcon = (provider: InferenceProvider) => {
    ...
    if (!health) return <ExclamationTriangleIcon className="w-4 h-4 text-yellow-500" />;
      ...
      <div className="space-y-3">
        ...
        <div className="relative">
          ...
          >
            <div className="flex items-center space-x-3">
              ...
              <div className="text-left">
                ...
                <div className="text-sm opacity-75">Multi-provider cloud inference</div>
              </div>
            </div>
            <div className="flex items-center space-x-2">
              {showHealthStatus && (
          ...
          {!isProviderAvailable('cloud') && (
            <div className="absolute inset-0 bg-gray-800/50 rounded-lg flex items-center justify-center">
              <span className="text-xs text-gray-400">No providers configured</span>
            </div>
          )}
        </div>
      </div>
        ...
        <div className="flex items-center space-x-2">
          ...
          {showHealthStatus && (
            <div className="flex items-center space-x-1 text-xs">
              ...
              <span className="text-gray-400">{getHealthText(currentProvider)}</span>
            </div>
          )}
        </div>
      </div>
