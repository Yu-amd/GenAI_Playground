...
export const GPUCloudManager: React.FC<GPUCloudManagerProps> = () => {
  ...
  const handleStartInstance = async (id: string) => {
    try {
      ...
      setError(
        `Failed to start instance: ${err instanceof Error ? err.message : 'Unknown error'}`
      );
    } finally {
      setLoading(false);
    }
  ...
  const handleStopInstance = async (id: string) => {
    try {
      ...
      setError(
        `Failed to stop instance: ${err instanceof Error ? err.message : 'Unknown error'}`
      );
    } finally {
      setLoading(false);
    }
        ...
        {instances.length === 0 ? (
          <div className='text-center py-12'>
            ...
            <p className='text-sm text-gray-500'>
              Use the Deploy Instances tab to create new GPU instances
            </p>
          </div>
        ) : (
              ...
              >
                <div className='flex items-start justify-between mb-3'>
                  <div>
                    ...
                    <p className='text-sm text-gray-400'>
                      {instance.provider} • {instance.gpuType}
                    </p>
                  </div>
                  <div className='flex items-center space-x-2'>
                    {getStatusIcon(instance.status)}
                    <span
                      className={`text-sm font-medium ${getStatusColor(instance.status)}`}
                    >
                      {instance.status}
                    </span>
                  </div>
                ...
                <div className='grid grid-cols-2 gap-4 mb-4'>
                  ...
                  <div>
                    ...
                    <p className='text-sm text-white'>
                      {instance.totalRuntime.toFixed(1)}h
                    </p>
                  </div>
                </div>
