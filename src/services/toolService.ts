...
export class ToolService {
  ...
  private async getStockPrice(args: {
    symbol: string;
    include_currency?: boolean;
    date?: string;
  }): Promise<string> {
    ...
    try {
      ...
      const isHistoricalQuery =
        date ||
        symbol.toLowerCase().includes('on') ||
        symbol.toLowerCase().includes('june') ||
        symbol.toLowerCase().includes('2025');
  ...
  private async getHistoricalStockPrice(
    symbol: string,
    date?: string
  ): Promise<string> {
    try {
      ...
      if (targetDate.match(/^\d{4}-\d{2}-\d{2}$/)) {
        ...
        const dateMatch = targetDate.match(
  ...
  private getMockStockPrice(symbol: string, includeCurrency: boolean): string {
    ...
    > = {
      ...
      GOOGL: { price: 2750.8, change: -15.2, percent: '-0.55%' },
      MSFT: { price: 320.45, change: 5.3, percent: '+1.68%' },
      TSLA: { price: 245.9, change: -8.75, percent: '-3.44%' },
      AMZN: { price: 135.6, change: 1.25, percent: '+0.93%' },
    };
