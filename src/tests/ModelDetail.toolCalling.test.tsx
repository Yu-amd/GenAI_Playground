    ...
    const MockToolSelector = () => {
      return (
        <div data-testid='tool-selector'>
          ...
          <button onClick={() => onToolToggle('get_weather')}>
            Toggle Weather
          </button>
...
vi.mock('../components/ToolTestPanel', () => ({
  default: ({
    ...
    <div data-testid='tool-test-panel'>
      ...
      <div data-testid='tool-calling-enabled'>
        {isToolCallingEnabled.toString()}
      </div>
      <button onClick={() => onTestMessage('What is the weather in New York?')}>
        Test Weather
      </button>
      <button onClick={() => onTestMessage('Calculate 2 + 2')}>
        Test Calculate
      </button>
    </div>
  ),
...
describe('ModelDetail Tool Calling', () => {
  beforeEach(() => {
    ...
    // Mock toolService
    (
      toolService as unknown as { executeTool: ReturnType<typeof vi.fn> }
  ...
  describe('Tool Selection', () => {
    ...
    it('should enable all tools when enable all is clicked', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByTestId('enabled-tools-count')).toHaveTextContent(
          '6'
        );
      });
    });
  ...
  describe('Tool Documentation', () => {
    ...
    it('should close tool documentation modal when close button is clicked', async () => {
      ...
      await waitFor(() => {
        expect(
          screen.queryByTestId('tool-documentation')
  ...
  describe('Tool Test Panel', () => {
    ...
    it('should show correct tool calling status in test panel', async () => {
      ...
      expect(screen.getByTestId('tool-calling-enabled')).toHaveTextContent(
        'true'
      );
    });
    ...
    it('should show enabled tools in test panel', async () => {
      ...
      expect(screen.getByTestId('enabled-tools-test')).toHaveTextContent(
        'get_weather,search_web,calculate,translate,get_stock_price,get_time'
      );
    });
  ...
  describe('Tool Calling Execution', () => {
    ...
    it('should execute tool calls when received from model', async () => {
      ...
      ).chatCompletion = vi.fn().mockResolvedValue(mockToolCallResponse);
    ...
    it('should add tool response to messages after execution', async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByText('Weather data: Sunny, 75°F')
        ).toBeInTheDocument();
      });
    });
    ...
    it('should handle multiple tool calls in sequence', async () => {
      ...
      ).chatCompletion = vi.fn().mockResolvedValue(multiToolResponse);
  ...
  describe('Tool Calling Error Handling', () => {
    it('should handle tool execution errors gracefully', async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByText('I will check the weather for you.')
        ).toBeInTheDocument();
      });
    });
