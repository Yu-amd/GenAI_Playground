    ...
    try {
      ...
      if (azureKey && azureEndpoint) {
        await cloudConfigService.addProvider({
          ...
          apiKey: azureKey,
          config: {
            apiVersion: process.env.REACT_APP_AZURE_API_VERSION || '2024-02-15-preview'
          },
          priority: 2,
          enabled: true
        });
      ...
      alert('Sample providers added! Check the Cloud Provider Manager to configure them.');
    } catch (error) {
    ...
    try {
      ...
      for (const provider of providers) {
        try {
          ...
          results.push(`${provider}: ${isHealthy ? '✅ Healthy' : '❌ Unhealthy'}`);
        } catch (error) {
          results.push(`${provider}: ❌ Error - ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
      }
      ...
      <div className="max-w-6xl mx-auto space-y-8">
        ...
        <div className="text-center">
          ...
          <p className="text-gray-400">
            Demonstrate cloud inference with multiple providers and seamless switching
          </p>
        </div>
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="flex items-center justify-between">
                  <div className="text-sm text-gray-400">
                    Current Provider: <span className="text-blue-400">{currentProvider}</span>
                  </div>
                  <button
                    type="submit"
