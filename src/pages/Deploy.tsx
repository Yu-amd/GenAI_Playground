...
const cloudProviders = {
  'amd-developer-cloud': {
    ...
    description:
      'Access the latest AMD Instinct™ accelerators in a pre-configured environment designed for developers.',
    icon: FaMicrochip,
    color: 'text-blue-400',
  ...
  'oracle-cloud-infrastructure': {
    ...
    description:
      'Enterprise-grade cloud infrastructure with AMD Instinct™ MI300X GPUs for high-performance AI workloads.',
    icon: FaCloud,
    color: 'text-red-400',
  ...
  'microsoft-azure': {
    ...
    description:
      'Virtual machines featuring AMD Instinct MI210-series GPUs for HPC and AI applications.',
    icon: FaServer,
    color: 'text-blue-500',
  ...
  vultr: {
    name: 'Vultr',
    description:
      'Global cloud platform offering AMD Instinct™ MI325X and MI300X GPUs for AI and HPC workloads.',
    icon: FaRocket,
  ...
  'hot-aisle': {
    name: 'Hot Aisle',
    description:
      'Bare metal access to AMD MI300X enterprise accelerators with on-demand provisioning.',
    icon: FaShieldAlt,
  ...
  tensorwave: {
    name: 'TensorWave',
    description:
      'AI-optimized infrastructure leveraging next-generation AMD accelerators for scalable AI workloads.',
    icon: FaChartLine,
...
const Deploy: React.FC = () => {
  ...
  const selectedConfig =
    cloudProviders[selectedProvider as keyof typeof cloudProviders];
        ...
        <div className='w-full max-w-[1400px] mx-auto py-12 px-8 flex-1'>
          ...
          <div className='text-center mb-12'>
            ...
            <p className='text-lg text-gray-300 mb-8'>
              Choose a cloud provider to deploy your AMD Instinct™ GPU
              workloads
            </p>
          </div>
          {/* Provider Dropdown */}
          <div className='max-w-md mx-auto mb-12'>
            <div className='relative'>
              ...
              {showDropdown && (
                <div
                  ref={dropdownRef}
                  className='absolute top-full left-0 right-0 mt-2 bg-gray-800 border border-white/20 rounded-xl overflow-hidden z-50 max-h-96 overflow-y-auto'
                >
                  {Object.entries(cloudProviders).map(([id, config]) => {
                    ...
                    return (
                      ...
                      >
                        <div className='flex items-center'>
                          <div
                            className={`w-10 h-10 ${config.bgColor} rounded-lg flex items-center justify-center mr-3`}
                          >
    ...
    mainContent = (
      <div className='min-h-screen bg-neutral-900 text-white flex items-center justify-center'>
        <div className='text-center'>
          ...
          <Link to='/deploy' className='text-blue-400 hover:underline'>
    ...
    mainContent = (
      <>
        ...
        <div className='w-full max-w-[1400px] mx-auto py-12 px-8 flex-1'>
          ...
          <div className='mb-8'>
            <div className='flex items-center justify-between mb-6'>
              <div className='flex items-center'>
                ...
                <div
                  className={`w-16 h-16 ${selectedConfig.bgColor} rounded-lg flex items-center justify-center mr-4`}
                >
                  <ProviderIcon
                    className={`text-2xl ${selectedConfig.color}`}
                  />
                </div>
                <div>
              ...
              <div className='relative'>
                <button
                  onClick={() => setShowDropdown(!showDropdown)}
                  onKeyDown={handleKeyDown}
                  className='bg-white/5 backdrop-blur-md border border-white/20 rounded-xl px-4 py-2 flex items-center hover:bg-white/10 transition-colors focus:outline-none focus:ring-2 focus:ring-blue-500'
                >
                  <div
                    className={`w-8 h-8 ${selectedConfig.bgColor} rounded-lg flex items-center justify-center mr-3`}
                  >
                    <ProviderIcon
                      className={`text-lg ${selectedConfig.color}`}
                    />
                  </div>
                ...
                {showDropdown && (
                  <div
                    ref={dropdownRef}
                    className='absolute top-full right-0 mt-2 bg-gray-800 border border-white/20 rounded-xl overflow-hidden z-50 min-w-80'
                  >
                    {Object.entries(cloudProviders).map(([id, config]) => {
                      const Icon = config.icon;
                      return (
                        <button
                          key={id}
                          onClick={() => handleProviderSelect(id)}
                          className={`w-full p-3 text-left hover:bg-white/10 transition-colors border-b border-white/10 last:border-b-0 focus:outline-none focus:bg-white/10 ${id === selectedProvider ? 'bg-white/5' : ''} ${focusedIndex === Object.keys(cloudProviders).indexOf(id) ? 'bg-white/10' : ''}`}
                        >
                          <div className='flex items-center'>
                            <div
                              className={`w-8 h-8 ${config.bgColor} rounded-lg flex items-center justify-center mr-3`}
                            >
          ...
          {/* Deploy Tab */}
          <div
            className={`${selectedConfig.bgColor} ${selectedConfig.borderColor} border rounded-2xl p-8 mb-8`}
          >
            <DeployTab
              ...
              customHeader={
                <div className='mb-6'>
                  <h2 className='text-2xl font-bold text-white mb-2'>
                    Deploy to {selectedConfig.name}
                  </h2>
                  <p className='text-gray-300'>
                    Configure and deploy your AMD Instinct™ GPU workloads
                  </p>
                </div>
              }
              customFooter={
