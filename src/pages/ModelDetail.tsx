...
const ModelCardViewContent: React.FC<{ modelCard: ModelCardContent }> = ({
  modelCard,
}) => {
  if (!modelCard) {
    return (
      <div className='text-center py-12 text-gray-400'>
        No model card information available.
      </div>
    );
  }
  ...
  const renderSection = (
    sectionKey: string,
    sectionData: string | readonly string[] | string[]
  ) => {
  ...
  return (
    <>
      ...
      {Object.keys(modelCard).map(sectionKey =>
        renderSection(sectionKey, modelCard[sectionKey])
      )}
    </>
  );
};
...
const ModelCardEditingContent: React.FC<{
    ...
    if (currentIndex > 0) {
      ...
      sectionKeys.forEach((key, index) => {
        if (index === currentIndex - 1) {
          ...
          newContent[sectionKeys[currentIndex - 1]] =
            editingContent[sectionKeys[currentIndex - 1]];
        } else {
            ...
            {sectionData.map((item: string, index: number) => (
              <div key={index} className='flex items-center gap-3'>
                <input
                  ...
                  value={item}
                  onChange={e =>
                    updateListItem(sectionKey, index, e.target.value)
                  }
  ...
  const allSections = Object.keys(editingContent).filter(
    key => editingContent[key] !== undefined
  );
  ...
  return (
    <div className='space-y-6'>
              ...
              <div className='flex items-center gap-4'>
                <label className='flex items-center gap-2 text-gray-200'>
                  <input
                    ...
                    onChange={e =>
                      setNewSectionType(e.target.value as 'text' | 'list')
                    }
      ...
      {allSections.map(sectionKey =>
        renderSection(sectionKey, editingContent[sectionKey])
      )}
...
const ModelDetail: React.FC = () => {
  ...
  const [modelCardContent, setModelCardContent] =
    useState<ModelCardContent | null>(null);
  ...
  const [parameters, setParameters] = useState<Parameter[]>([
    {
      ...
      type: 'number',
      description:
        'Controls randomness in the output (0.0 = deterministic, 2.0 = very random)',
      min: 0,
      max: 2,
      step: 0.1,
    },
    {
      ...
      type: 'number',
      description:
        'Controls diversity via nucleus sampling (0.0 = focused, 1.0 = diverse)',
      min: 0,
      max: 1,
      step: 0.1,
    },
    ...
    },
    {
      ...
      value: 0.0,
      type: 'number',
      description: 'Encourages model to talk about new topics (-2.0 to 2.0)',
      min: -2,
      max: 2,
      step: 0.1,
    },
  ...
  const [tools] = useState([
        ...
        parameters: {
          ...
          properties: {
            ...
            source_language: {
              type: 'string',
              description:
                'The source language code (optional, will auto-detect if not provided)',
            },
          },
    ...
    },
    {
      ...
      function: {
        ...
        description:
          'Get the current stock price for a ticker symbol or historical price for a specific date',
        parameters: {
          type: 'object',
          properties: {
            symbol: {
            ...
            date: {
              type: 'string',
              description:
                'Optional date for historical price (e.g., "2025-06-20" or "June 20, 2025")',
            },
          },
          required: ['symbol'],
        },
  ...
  useEffect(() => {
    setCodeContent(
      getDefaultCode(
        ...
        {
          temperature: parameters.find(p => p.name === 'temperature')
            ?.value as number,
          max_tokens: parameters.find(p => p.name === 'max_tokens')
            ?.value as number,
        ...
        value:
          param.name === 'temperature'
            ...
            : param.name === 'max_tokens'
              ...
              : param.name === 'top_p'
                ? 0.9
                : param.name === 'frequency_penalty'
                  ? 0.0
                  : param.name === 'presence_penalty'
                    ? 0.0
                    : param.name === 'max_tokens'
                      ? 2048
                      : param.value,
  ...
  const handleSendMessage = async () => {
    ...
    try {
      const enabledTools = tools.filter(t =>
        enabledToolNames.includes(t.function.name)
      );
        ...
        messages: [
          {
            ...
            content:
              'You have access to various tools. When a user asks for information that requires real-time data, calculations, translations, or other external services, use the appropriate tools to help them. Always use tools when they would be helpful.',
          },
      ...
      >
        ...
        <div className='w-full flex justify-center z-50 mb-6'>
          <div className='flex items-start gap-3 bg-white/20 backdrop-blur-md border border-black text-white px-6 py-2 rounded-xl shadow-md w-[90%] text-sm drop-shadow font-normal'>
            ...
            <span>
              AI models generate responses and outputs based on complex
              algorithms and machine learning techniques, and those responses or
              outputs may be inaccurate, harmful, biased or indecent.{' '}
        ...
        <div className='flex flex-col md:flex-row max-w-[1600px] mx-auto p-8 gap-8 -mt-8 relative z-10'>
          <div className='flex-1'>
            <div
              className='flex flex-row gap-8 items-stretch h-[700px]'
              style={{ position: 'relative' }}
            >
              {/* Chatbox (left) - Always rendered */}
              <div
                className='w-full md:w-[40%] flex-1 h-full min-h-0 bg-black rounded-3xl p-4 shadow-lg mb-8 md:mb-0 flex flex-col font-sans'
                style={{ fontFamily: 'Inter, system-ui, sans-serif' }}
              >
                <div className='flex items-center justify-between mb-4'>
                  <div className='flex items-center gap-2'>
                  ...
                  <div className='flex items-center gap-2'>
                    ...
                    >
                      <input
                        ...
                        onChange={e =>
                          setIsToolCallingEnabled(e.target.checked)
                        }
                        className='rounded border-gray-600 bg-gray-800 text-blue-500 focus:ring-blue-500'
                      />
                      <span
                        className={isToolCallingEnabled ? 'font-medium' : ''}
                      >
                        {isToolCallingEnabled
                          ? 'Tool Calling Active'
                          : 'Enable Tool Calling'}
              ...
              <div className='w-full md:w-[60%] flex-1 h-full min-h-0 rounded-2xl border border-white/10 shadow-2xl flex flex-col api-integration-container'>
                ...
                <div className='flex-1 flex items-center justify-center'>
                  <div className='text-center text-gray-400'>
                    ...
                    <div className='text-sm'>
                      Code generation will be available once the model loads
                    </div>
                  </div>
                </div>
              </div>
    ...
    >
          ...
          >
            ...
            >
              ...
              {showParameters && (
                ...
                >
                  <div
                    className='bg-white/10 backdrop-blur-xl border border-white/20 rounded-l-3xl shadow-2xl w-full max-w-md h-full p-8 overflow-y-auto relative'
            ...
            <div
              className='w-full md:w-[60%] flex-1 h-full min-h-0 rounded-2xl border border-white/10 shadow-2xl flex flex-col'
            ...
            >
              ...
              >
                <div
                  className='rounded-xl overflow-hidden absolute inset-0 m-6'
                ...
                >
                  ...
                  >
                    {({
                      ...
                      <pre
                        className={`${className} p-6 text-sm font-mono whitespace-pre-wrap break-words overflow-x-auto overflow-y-auto h-full`}
                        style={{
                          ...style,
      ...
      >
        <div className='flex items-center justify-center min-h-screen px-4'>
          ...
          />
          <div
            className='relative bg-white/10 backdrop-blur-xl rounded-3xl shadow-2xl max-w-4xl w-full mx-auto p-8 z-10 border border-white/20'
            onClick={e => e.stopPropagation()}
          >
            ...
            <div className='overflow-x-auto overflow-y-auto max-h-[70vh] bg-black/40 rounded-xl border border-white/10'>
              ...
              >
                {({
                  ...
                  <pre
                    className={`${className} p-6 text-sm font-mono whitespace-pre break-words overflow-x-auto overflow-y-auto max-h-[70vh]`}
      ...
      {showModelCard && (
        ...
        >
          <div
            className='relative bg-white/5 backdrop-blur-xl rounded-2xl shadow-2xl text-lg border border-white/10 p-8 w-full max-w-4xl mx-auto overflow-y-auto max-h-[70vh]'
            onClick={e => e.stopPropagation()}
          >
            ...
            {isModelCardEditing && isDevelopment ? (
              ...
              <ModelCardViewContent
                modelCard={modelCardContent || model.model_card}
              />
            )}
          </div>
        </div>
      ...
      {showSettings && isDevelopment && (
        ...
        >
          <div
            className='relative bg-white/10 backdrop-blur-xl border border-white/20 rounded-2xl shadow-2xl w-full max-w-md p-8 flex flex-col items-center'
            onClick={e => e.stopPropagation()}
          >
            <button
      ...
      {showToolDocumentation && (
        <ToolDocumentation onClose={() => setShowToolDocumentation(false)} />
      )}
      ...
      {showDeploymentGuide && model && (
        <DeploymentGuide
          ...
          modelInfo={{
            ...
            size: model.size || '7B',
            requirements:
              model.size && model.size.includes('B')
